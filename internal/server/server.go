...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	// Hosts CRUD (kept under /api/hosts since that's what the inventory
	// is colloquially called, even though the table is named "servers")
	s.mux.HandleFunc("GET /api/hosts", s.list)
	s.mux.HandleFunc("POST /api/hosts", s.create)
	s.mux.HandleFunc("GET /api/hosts/{id}", s.get)
	s.mux.HandleFunc("PUT /api/hosts/{id}", s.update)
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{
			"tier":        s.limits.Tier,
			"upgrade_url": "https://stockyard.dev/homestead/",
		})
	})
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("homestead: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("homestead: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"hosts": oe(s.db.Search(q, filters))})
		return
	}
...
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	if s.limits.MaxItems > 0 && s.db.Count() >= s.limits.MaxItems {
		we(w, 402, "Free tier limit reached. Upgrade at https://stockyard.dev/homestead/")
		return
	}
	var e store.Host
...
// update accepts a partial host. The original update handler used the
// empty-string preserve pattern across all 8 fields, silently nuking
// every field except whatever was in the partial PUT.
func (s *Server) update(w http.ResponseWriter, r *http.Request) {
	...
	if v, ok := raw["provider"]; ok {
		json.Unmarshal(v, &patch.Provider)
	}
	if v, ok := raw["region"]; ok {
		json.Unmarshal(v, &patch.Region)
	}
	if v, ok := raw["tags"]; ok {
